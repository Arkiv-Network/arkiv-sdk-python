...
class TestAsyncEntityGet:
    ...
    async def test_async_get_entity_basic(
        ...
        assert (
            entity.owner == async_arkiv_client_http.eth.default_account
    ...
    async def test_async_get_entity_field_flags(
        ...
        assert (
            entity_metadata.expires_at_block is not None
        ), "Expiration should be retrieved"
        assert (
            entity_metadata.annotations is None
        ...
        assert (
            entity_annotations.annotations == annotations
        ...
        assert entity_all.annotations == annotations, "All: Annotations should be retrieved"
        assert (
            entity_all.expires_at_block is not None
    ...
    async def test_async_get_entities_concurrently(
        ...
        tasks = [
            async_arkiv_client_http.arkiv.get_entity(key) for key in entity_keys
        ]
        ...
        for i, entity in enumerate(entities):
            ...
            assert (
                entity.payload == f"Concurrent read test entity {i}".encode()
